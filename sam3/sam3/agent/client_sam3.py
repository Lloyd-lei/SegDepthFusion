...
def call_sam_service(
    ...
    # Create a safe directory name from the image path
    # Replace drive letter colon and slashes with hyphens to avoid path issues
    safe_image_name = image_path.replace(":", "").replace("/", "-").replace("\\", "-")
    
    os.makedirs(
        os.path.join(output_folder_path, safe_image_name), exist_ok=True
    )
