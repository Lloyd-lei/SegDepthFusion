...
def agent_inference(
    ...
    if debug:
        # Use os.path.basename and splitext for robust cross-platform filename extraction
        img_filename = os.path.basename(img_path)
        img_name_no_ext = os.path.splitext(img_filename)[0]
        
        debug_folder_path = os.path.join(
            debug_save_dir, img_name_no_ext
        )
    ...
    while generated_text is not None:
        ...
        # Try to find <tool> tag
        if "<tool>" not in generated_text:
             # Try to check if the JSON is wrapped in markdown code blocks like ```json ... ```
            import re
            json_match = re.search(r'```json\s*(\{.*?\})\s*```', generated_text, re.DOTALL)
            if json_match:
                # Fake a tool tag wrapper so the rest of logic works
                generated_text = f"<tool>{json_match.group(1)}</tool>"
            else:
                 # Fallback: assume the whole text might be JSON if it starts with {
                clean_text = generated_text.strip()
                if clean_text.startswith("{") and clean_text.endswith("}"):
                     generated_text = f"<tool>{clean_text}</tool>"
        ...
        try:
            tool_call = json.loads(tool_call_json_str)
             ...
             # Attempt to fix common JSON errors from LLMs (like single quotes)
            try:
                import ast
                tool_call = ast.literal_eval(tool_call_json_str)
            except:
                 # Try to extract the first JSON object found in the string using regex
                import re
                json_match = re.search(r'(\{.*\})', tool_call_json_str, re.DOTALL)
                if json_match:
                    try:
        ...
        if tool_call["name"] == "segment_phrase":
            ...
            for i in range(num_masks):
                ...
                verdict = ""
                if "<verdict>" in checking_generated_text and "</verdict>" in checking_generated_text:
                    verdict = (
                    ...
                    # Fallback: heuristic check if tags are missing
                    print("⚠️ Warning: <verdict> tags missing. Attempting heuristic extraction.")
                    lower_text = checking_generated_text.lower()
                    # Look at the last few sentences
                    last_part = lower_text[-500:]
                    if "incorrect" in last_part or "reject" in last_part or "does not match" in last_part or "no match" in last_part:
                        verdict = "Reject"
                    elif "correct" in last_part or "accept" in last_part or "matches" in last_part or "is the target" in last_part:
                        verdict = "Accept"
                    else:
                        # If totally ambiguous, default to Reject to be safe, or re-prompt (simplified here)
                        print("⚠️ Could not determine verdict heuristically. Defaulting to Reject.")
                        verdict = "Reject"

                if "Accept" in verdict or "accept" in verdict.lower():
                    print(f"Mask {i+1} accepted, keeping it in the outputs.")
                    masks_to_keep.append(i)
                elif "Reject" in verdict or "reject" in verdict.lower():
                    print(f"Mask {i+1} rejected, removing it from the outputs.")
                else:
                    print(f"⚠️ Unexpected verdict '{verdict}'. Treating as Reject.")
